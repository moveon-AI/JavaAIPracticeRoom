...
import cn.zzuli.mapper.AnswerRecordMapper;
import cn.zzuli.mapper.ExamRecordMapper;
import cn.zzuli.service.AnswerRecordService;
import cn.zzuli.service.ExamService;
import cn.zzuli.service.KimiAiService;
import cn.zzuli.service.PaperService;
...
public class ExamServiceImpl extends ServiceImpl<ExamRecordMapper, ExamRecord> implements ExamService {
    ...
    public void customRemoveById(Integer id) {
        //重要的关联数据校验，有删除失败！
        //判断自身状态，进行中不能删除
        ExamRecord examRecord = getById(id.longValue());
        if ("进行中".equals(examRecord.getStatus())){
            throw new RuntimeException("正在考试中，无法直接删除！");
        }
        // 删除自身数据，同时删除答题记录
        // 确保 id 类型正确 - ExamRecord的ID是Long类型
